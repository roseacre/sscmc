	<div class="row">
		<div class="col-md-1"></div>
		<div class="col-md-11">
			<h2 class="padded" id="pastoral-team">
				Our Pastoral Team
			</h2>
		</div>
	</div>

	<div class="row">
	    <div class="col-md-1"></div>
		<div class="col-md-3">
			<div class="card d-flex h-100 align-items-stretch">
				<img src="../images/janet-pybon.jpg" class="card-img-top"
					alt="Reverend Janet Pybon"
					title="Reverend Janet Pybon">
				<div class="card-body">
					<h3 class="card-title general-text font-weight-bold">Rev. Janet Pybon</h3>
					<p class="card-text card-sub-heading">Superintendent Minister</p>
				</div>
				<div class="card-body align-items-end">
					<p class="card-text card-sub-heading">Telephone</p>
					<p class="card-text"><a href="tel:01253302895">01253 302895</a></p>
					<p class="card-text card-sub-heading">Email</p>
					<p class="card-text"><a href="mailto:pybon@btinternet.com">pybon@btinternet.com</a></p>
					<p class="card-text card-sub-heading">Normal rest day</p>
					<p class="card-text">variable</p>
				</div>
			</div>
		</div>

		<div class="col-md-3">
			<div class="card d-flex h-100 align-items-stretch">
				<img src="../images/john_morton.png" class="card-img-top"
					alt="Reverend John Morton"
					title="Reverend John Morton">
				<div class="card-body">
					<h3 class="card-title general-text font-weight-bold">Rev. John Morton</h3>
					<p class="card-text card-sub-heading">Minister</p>
				</div>
				<div class="card-body align-items-end">
					<p class="card-text card-sub-heading">Telephone</p>
					<p class="card-text"><a href="tel:01253695527">01253 695527</a></p>
					<p class="card-text card-sub-heading">Email</p>
					<p class="card-text"><a href="mailto:johnmorton148@yahoo.co.uk">johnmorton148@yahoo.co.uk</a></p>
					<p class="card-text card-sub-heading">Normal rest day</p>
					<p class="card-text">Friday</p>
				</div>
			</div>
		</div>

		<div class="col-md-3">
			<div class="card d-flex h-100 align-items-stretch">
				<img src="../images/temporary.jpg" class="card-img-top" alt="Reverend Barrie Lees">
				<div class="card-body">
					<h3 class="card-title general-text font-weight-bold">Rev. Barrie Lees</h3>
					<p class="card-text card-sub-heading">Supernumerary</p>
				</div>
				<div class="card-body">
					<p class="card-text card-sub-heading">Telephone</p>
					<p class="card-text"><a href="tel:01253367966">01253 367966</a></p>
				</div>
			</div>
		</div>
		<div class="col-md-2"></div>
    </div>

